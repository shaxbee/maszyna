...
class TTrain
{
  ...
  public: // reszta może być publiczna
    ...
    TButton btLampkaForward; // Ra: lampki w przód i w tył dla komputerowych kabin
  ...
  private:
    ...
    float fTachoTimer;
    float fTachoCount;
    float fHVoltage; // napięcie dla dynamicznych gałek
    float fHCurrent[4]; // prądy: suma i amperomierze 1,2,3
    float fEngine[4]; // obroty też trzeba pobrać
    // McZapkie: do syczenia
    float fPPress, fNPress;
    float fSPPress, fSNPress;
    int iSekunda; // Ra: sekunda aktualizacji prędkości
