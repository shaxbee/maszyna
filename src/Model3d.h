...
{   // klasa submodelu - pojedyncza siatka, punkt świetlny albo grupa punktów
  ...
  private:
    ...
    // *Vertices);
    int SeekFaceNormal(std::uint32_t *Masks, int f, std::uint32_t dwMask, float3 *pt,
                       float8 *Vertices);
  ...
  public:
    ...
    TSubModel *GetFromName(const std::string search, bool i = true);
    void RenderDL();
    void RenderAlphaDL();
    void RenderVBO();
    void RenderAlphaVBO();
...
class TModel3d : public CMesh
{
  ...
  public:
    ...
    ~TModel3d();
    TSubModel *GetFromName(const std::string name);
