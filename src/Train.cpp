    ...
    { // wciśnięty [Shift]
        ...
        { // Ra: dźwięk, gdy razem z [Shift]
            if ((mvControlled->TrainType & dt_EZT) ? mvControlled == mvOccupied :
                ...
                if (Console::Pressed(VK_CONTROL)) // z [Ctrl]
                    mvControlled->bPantKurek3 = true; // zbiornik pantografu połączony jest ze
                                                      // zbiornikiem głównym (pompowanie nie ma
                                                      // sensu)
        ...
        { // McZapkie-240302 - wlaczanie automatycznego pilota (zadziala tylko w trybie debugmode)
            if (DynamicObject->Mechanik)
            {
                if (DebugModeFlag)
                    if (DynamicObject->Mechanik
                            ->AIControllFlag) //żeby nie trzeba było rozłączać dla zresetowania
        ...
        else if (cKey == Global::Keys[k_MaxCurrent]) // McZapkie-160502: F - wysoki rozruch
        {
            ...
            /* Ra: przeniesione do Mover.cpp
                   if (mvControlled->TrainType!=dt_EZT) //to powinno być w fizyce, a nie w kabinie!
        ...
        { // Winger 160204: podn. przedn. pantografu
            ...
            { // przedni gdy w kabinie 1 lub (z wyjątkiem ET40, ET41, ET42 i EZT) gdy w kabinie -1
        ...
        { // Winger 160204: podn. tyln. pantografu względem kierunku jazdy
            if (mvOccupied->ActiveCab ==
                1) //||((mvOccupied->ActiveCab<1)&&((mvControlled->TrainType&(dt_ET40|dt_ET41|dt_ET42|dt_EZT))==0)))
            { // tylny gdy w kabinie 1 lub (z wyjątkiem ET40, ET41, ET42 i EZT) gdy w kabinie -1
                mvControlled->PantRearSP = false;
                if (mvControlled->PantRear(true))
                    if (mvControlled->PantRearStart != 1)
                    {
        ...
        { // Ra 2014-06: uruchomiłem to, aby aktywować czuwak w zajmowanym członie, a wyłączyć w
          // innych
            // Ra 2014-03: aktywacja czuwaka przepięta na ustawienie kierunku w mvOccupied
            // if (mvControlled->Battery) //jeśli bateria jest już załączona
            // mvOccupied->BatterySwitch(true); //to w ten oto durny sposób aktywuje się CA/SHP
        ...
        { // Ra 2014-09: w trybie latania obsługa jest w World.cpp
            ...
            if (cKey == Global::Keys[k_LeftSign]) // lewe swiatlo - włączenie
        {
            if ((GetAsyncKeyState(VK_CONTROL) < 0) &&
        ...
        else if (cKey == Global::Keys[k_UpperSign]) // ABu 060205: światło górne - włączenie
        {
            if ((GetAsyncKeyState(VK_CONTROL) < 0) &&
    ...
    else // McZapkie-240302 - klawisze bez shifta
    {
        ...
        { // Ra 2014-06: wersja dla swobodnego latania przeniesiona do World.cpp
            if (!FreeFlyModeFlag)
            {
                if (GetAsyncKeyState(VK_CONTROL) < 0)
                    if ((mvOccupied->LocalBrake == ManualBrake) || (mvOccupied->MBrake == true))
                        ...
                        ;
                else // Ra 1014-06: AI potrafi zahamować pomocniczym mimo jego braku - odhamować
                     // jakoś trzeba
            ...
            if (mvOccupied->BrakeLevelAdd(
                    Global::fBrakeStep)) // nieodpowiedni warunek; true, jeśli można dalej kręcić
            {
                keybrakecount = 0;
        ...
        else if ((cKey == Global::Keys[k_DecBrakeLevel]) && (mvOccupied->BrakeHandle != FV4a))
        {
            ...
            /* wersja poprzednia - ten pierwszy if ze średnikiem nie działał jak warunek
                     if ((mvOccupied->BrakeCtrlPos>-1)|| (keybrakecount>1))
                      {
        ...
        { // Ra: bez [Shift] też dać dźwięk
            if ((mvControlled->TrainType & dt_EZT) ? mvControlled == mvOccupied :
                                                     !mvOccupied->ActiveCab) // tylko w maszynowym
                if (Console::Pressed(VK_CONTROL)) // z [Ctrl]
                    mvControlled->bPantKurek3 = false; // zbiornik pantografu połączony jest z małą
                                                       // sprężarką (pompowanie ma sens, ale potem
                                                       // trzeba przełączyć)
                else if (!mvControlled->PantCompFlag) // jeśli wyłączona
                    if (mvControlled->Battery) // jeszcze musi być załączona bateria
                        if (mvControlled->PantPress < 4.8) // piszą, że to tak nie działa
        ...
        { // ABu051104: male zmiany, zeby mozna bylo rozlaczac odlegle wagony
            if (iCabn > 0)
            {
                if (!FreeFlyModeFlag) // tryb 'kabinowy' (pozwala również rozłączyć sprzęgi
                                      // zablokowane)
                {
        ...
        else if (cKey == Global::Keys[k_UpperSign]) // ABu 060205: światło górne - wyłączenie
        {
            if ((GetAsyncKeyState(VK_CONTROL) < 0) &&
        ...
        else
        {
            ...
            if (!Console::Pressed(VK_CONTROL)) // gdy [Ctrl] zwolniony (dodatkowe widoki)
            {
                if (cKey == Global::Keys[k_MechLeft])
                {
                    ...
                    if (DynamicObject->Mechanik)
                        if (!FreeFlyModeFlag) //żeby nie mieszać obserwując z zewnątrz
                            DynamicObject->Mechanik->RouteSwitch(1); // na skrzyżowaniu skręci w
                                                                     // lewo
                }
                ...
                else if (cKey == Global::Keys[k_MechBackward])
                {
                    ...
                    // if (DynamicObject->Mechanik)
                    // if (!FreeFlyModeFlag) //żeby nie mieszać obserwując z zewnątrz
                    //  DynamicObject->Mechanik->RouteSwitch(0);  //na skrzyżowaniu stanie i poczeka
                }
...
{ // Ra: mechanik powinien być telepany niezależnie od pozycji pojazdu
    ...
    { // hamowanie rzucania przy spadku FPS
        pMechShake -=
            pMechShake *
            Min0R(dt, 1); // po tym chyba potrafią zostać jakieś ułamki, które powodują zjazd
        pMechOffset += vMechMovement * dt;
...
bool TTrain::Update()
{
    ...
    { // Ra: TODO: odczyty klawiatury/pulpitu nie powinny być uzależnione od istnienia modelu kabiny
        ...
        */
        ...
        // PoKeys
        if (mvControlled->EngineType != DieselElectric) // Ra 2014-09: czy taki rozdział ma sens?
        ...
        { // wykonywać tylko gdy wyprowadzone na pulpit
            ...
            Console::ValueSet(6, fTachoVelocity); ////Ra: prędkość na pin 43 - wyjście analogowe (to
        ...
        */
        ...
        //McZapkie-240302    ggVelocity.UpdateValue(DynamicObject->GetVelocity());
            ...
            {//Ra: ryzykowne jest to, gdyż może się nie uaktualniać prędkość
             //Ra: prędkość się powinna zaokrąglać tam gdzie się liczy fTachoVelocity
        ...
        */
        ...
        { // Ra 2014-12: lokomotywy 181/182 dostają SlippingWheels po zahamowaniu powyżej 2.85 bara
          // i buczały
            double veldiff = (DynamicObject->GetVelocity() - fTachoVelocity) / mvControlled->Vmax;
            if (veldiff <
                -0.01) // 1% Vmax rezerwy, żeby 181/182 nie buczały po zahamowaniu, ale to proteza
            {
        ...
        if (mvControlled->Mains)
        {
            ...
            if ((mvControlled->Itot != 0) || (mvOccupied->BrakePress > 2) ||
                ...
                btLampkaStyczn.TurnOff(); // Ra: czy to jest udawanie działania styczników
                                          // liniowych?
            ...
            // Ra: w SU45 boczniki wchodzą na MainCtrlPos, a nie na MainCtrlActualPos - pokićkał
            // ktoś?
        ...
        { // yB - wskazniki drugiego czlonu
            TDynamicObject *
                tmp; //=mvControlled->mvSecond; //Ra 2014-07: trzeba to jeszcze wyjąć z kabiny...
            ...
            { // Ra: nie najlepsze miejsce, ale na początek gdzieś to dać trzeba
                ...
                if ((b >= 0.0) && ((mvOccupied->BrakeHandle == FV4a) ||
                                   (mvOccupied->BrakeHandle ==
                                    FVel6))) // może można usunąć ograniczenie do FV4a i FVel6?
                {
        ...
        { // czuwak testuje kierunek, ale podobno w EZT nie, więc może być w rozrządczym
            fCzuwakTestTimer += dt;
            ggSecurityResetButton.PutValue(1);
            if (CAflag == false)
            {
...
{ // McZapkie-090902: zmiana kabiny 1->0->2 i z powrotem
    ...
    { // jeśli pojazd prowadzony ręcznie albo wcale (wagon)
        ...
        DynamicObject->MoverParameters->CabActivisation(); // aktywizacja poprzedniej, bo jeszcze
                                                           // nie wiadomo, czy jakiś pojazd jest
    }
    return false; // ewentualna zmiana pojazdu
}
...
bool TTrain::LoadMMediaFile(AnsiString asFileName)
{
    ...
    delete Parser; // Ra: jak się coś newuje, to trzeba zdeletować, inaczej syf się robi
...
bool TTrain::InitializeCab(int NewCabNo, AnsiString asFileName)
{
    ...
    str = "";
    int cabindex = 0;
    TGauge *gg; // roboczy wsaźnik na obiekt animujący gałkę
    ...
    if (cabindex != 1)
        if (AnsiCompareStr(str, cabstr) != 0) // jeśli nie znaleziony wpis kabiny
    ...
    if (AnsiCompareStr(str, cabstr) == 0) // jeśli znaleziony wpis kabiny
    {
        ...
        { // ABu: wstawione warunki, wczesniej tylko to:
            ...
            if (parse)
                ...
                parse = true;
            // inicjacja kabiny
            // Ra 2014-08: zmieniamy zasady - zamiast przypisywać submodel do istniejących obiektów
            // animujących
            // będziemy teraz uaktywniać obiekty animujące z tablicy i podawać im submodel oraz
            // wskaźnik na parametr
                ...
                0) // model kabiny
            {
                ...
                if (str != AnsiString("none"))
                {
                    ...
                    TModel3d *k = TModelsManager::GetModel(
                        str.c_str(), true); // szukaj kabinę jako oddzielny model
                    Global::asCurrentTexturePath =
                    ...
                    //(mdKabina) może zostać to samo po przejściu do innego członu bez zmiany
                    //kabiny, przy powrocie musi być wiązanie ponowne
                    // else
            ...
            { // manometr zbiornika kontrolnego/rorządu
                gg = Cabine[cabindex].Gauge(-1); // pierwsza wolna gałka
                gg->Load(Parser, DynamicObject->mdKabina, NULL, 0.1);
            ...
            else if (str == AnsiString("maingearstatus:")) // np. ciśnienie sterownika skrzyni
                                                           // biegów
    ...
    {
        DynamicObject->mdKabina
            ->Init(); // obrócenie modelu oraz optymalizacja, również zapisanie binarnego
        return true;
    }
...
{ // taka proteza: chcę podłączyć kabinę EN57 bezpośrednio z silnikowym, aby nie robić tego przez
  // ukrotnienie
    ...
    // problem się robi ze światłami, które będą zapalane w silnikowym, ale muszą świecić się w
    // rozrządczych
