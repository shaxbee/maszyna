...
bool TWorld::Init(HWND NhWnd, HDC hDC)
{
    ...
    while (glver.LastDelimiter(".") > glver.Pos("."))
        glver = glver.SubString(1, glver.LastDelimiter(".") - 1); // obcięcie od drugiej kropki
    double ogl;
    try
    {
        ogl = glver.ToDouble();
    ...
    else
    {
        Global::bDecompressDDS =
            !glewGetExtension("GL_EXT_texture_compression_s3tc"); // czy obsługiwane?
...
bool TWorld::Update()
{
    ...
    { // jak doszło do zera, to sprawdzamy wydajność
        ...
        else if (GetFPS() > Global::fFpsMax) // jeśli jest dużo FPS
        ...
        */
        iCheckFPS = 0.25 * GetFPS(); // tak za 0.25 sekundy sprawdzić ponownie (jeszcze przycina?)
    }
    ...
    double dt = GetDeltaRenderTime(); // nie uwzględnia pauzowania ani mnożenia czasu
    fTime50Hz +=
    ...
    { // tekst pokazywany po wciśnięciu [F1]
        ...
        if (Controlled)
            if (Controlled->Mechanik)
            {
                ...
                if (!OutText2.IsEmpty()) // jeśli jest podana relacja, to dodajemy punkt następnego
                                         // zatrzymania
                    OutText2 =
                ...
                { // o ile jest ktoś w środku
                    ...
                    for (int i = 0, j = 1; i < 19; ++i, j <<= 1)
                        if (tmp->Mechanik->DrivigFlags() & j) // jak bit ustawiony
                            flags[i + 1] ^= 0x20; // to zmiana na wielką literę
                    OutText4 = flags;
                    OutText4 +=
...
//---------------------------------------------------------------------------
AnsiString last; // zmienne używane w rekurencji
double shift = 0;
            ...
            if (sr.Name[1] != '.')
                ...
                else if (dyn)
                {
                    if (sr.Name.LowerCase().SubString(sr.Name.Length() - 3, 4) == ".mmd")
                    {
                        ...
                        loads = tmp->DynamicObject->MoverParameters->LoadAccepted; // typy ładunków
                        if (!loads.IsEmpty())
                        {
