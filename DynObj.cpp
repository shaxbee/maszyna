...
{ // skanowanie toru w poszukiwaniu kolidujących pojazdów
    ...
    int MyScanDir =
        ScanDir; // zapamiętanie kierunku poszukiwań na torze początkowym, względem sprzęgów
    ...
    */
    ...
    { // siebie można bezpiecznie podłączyć jednostronnie do znalezionego
        ...
        // Ra: jeśli dwa samochody się mijają na odcinku przed zawrotką, to odległość między nimi
...
{ // Ustawienie początkowe pojazdu
    ...
    // else if (DriverType=="connected") DriverType="c"; //tego trzeba się pozbyć na rzecz
    // ukrotnienia
    ...
    */
    ...
    iLights = MoverParameters
                  ->iLights; // wskaźnik na stan własnych świateł (zmienimy dla rozrządczych EZT)
...
{ // przesuwanie pojazdu po trajektorii polega na przesuwaniu poszczególnych osi
    ...
    { // liczenie pozycji pojazdu tutaj, bo jest używane w wielu miejscach
        ...
        // (wygasza drgania?)
        //{//parę centymetrów trzeba by już skorygować; te błędy mogą się też generować na ostrych
        //łukach
...
{ // Ra: doczepia Object na końcu składu (nazwa funkcji może być myląca)
    ...
    */
    ...
    if (MoverParameters->TrainType &
        dt_EZT) // w przypadku łączenia członów, światła w rozrządczym zależą od stanu w silnikowym
...
bool TDynamicObject::Update(double dt, double dt1)
{
    ...
    // Ra 15-01: przeliczenie poboru prądu powinno być robione wcześniej, żeby na tym etapie były
    // znane napięcia
    ...
    { // dla EZT tylko silnikowy
        ...
        { // Ra 2013-12: to niżej jest chyba trochę bez sensu
            ...
            if (v == 0.0)
            {
                ...
                if (v == 0.0)
                    if (MoverParameters->TrainType &
                        (dt_EZT | dt_ET40 | dt_ET41 | dt_ET42)) // dwuczłony mogą mieć sprzęg WN
        ...
        // else //Ra: nie no, trzeba podnieść pantografy, jak nie będzie drutu, to będą miały prąd
        // po osiągnięciu 1.4m
    ...
    { // przeliczanie kątów dla pantografów
        ...
        double fCurrent =
            ...
            MoverParameters
                ->TotalCurrent; // prąd pobierany przez pojazd - bez sensu z tym (TotalCurrent)
        // fCurrent+=fabs(MoverParameters->Voltage)*1e-6; //prąd płynący przez woltomierz,
        ...
        if (pants)
            if (iAnimType[ANIM_PANTS] >
                1) // a jeśli są dwa pantografy //Ra 1014-11: proteza, trzeba zrobić sensowniej
        ...
        { // pętla po wszystkich pantografach
            ...
            if (MoverParameters->PantPress >
                (MoverParameters->TrainType == dt_EZT ?
                     2.5 :
                     3.3)) // Ra 2013-12: Niebugocław mówi, że w EZT podnoszą się przy 2.5
                    ...
                    MoverParameters->PantFrontUp) // jeśli ma być podniesiony
            {
                ...
                { // jeśli poprzednia wysokość jest mniejsza niż pożądana, zwiększyć kąt dolnego
                  // ramienia zgodnie z ciśnieniem
                    ...
                    // jeśli przekroczono kąt graniczny, zablokować pantograf (wymaga interwencji
                    // pociągu sieciowego)
                }
        ...
        { // Ra: animacja rozrządu parowozu, na razie nieoptymalizowane
            /* //Ra: tymczasowo wyłączone ze względu na porządkowanie animacji pantografów
               ...
               {//obie strony w ten sam sposób
                ...
                fi=DegToRad(dWheelAngle[1]+(i?pant2x:pant1x)-96.77416667); //kąt obrotu koła dla
               tłoka 1
                //1) dla dWheelAngle[1]=0° korba jest w dół, a mimośród w stronę jarzma, czyli
               fi=-7°
                ...
                if (smPatykird2[i]) //na razie zabezpieczenie
                 smPatykird2[i]->SetRotateXYZ(vector3(RadToDeg(-gam-ksi),0,0)); //obrócenie drążka
               mimośrodowego
               }
            */
        }
...
{ // przeliczanie dźwięków, bo będzie słychać bez wyświetlania sektora z pojazdem
    ...
    if ((MoverParameters->MainCtrlPos) >=
        (MoverParameters
             ->TurboTest)) // hunter-250312: dlaczego zakomentowane? Ra: bo nie działało dobrze
    {
            ...
            while (!Parser->EndOfFile && str != AnsiString("endmodels"))
            {
                    ...
                    if (pants)
                        ...
                        { // przepisanie współczynników do pantografów (na razie nie będzie lepiej)
                            ...
                            if (pants[i].fParamPants->fHeight ==
                                0.0) // gdy jest nieprawdopodobna wartość (np. nie znaleziony ślizg)
                ...
                { // Ra: parametry silnika parowego (tłoka)
                    /* //Ra: tymczasowo wyłączone ze względu na porządkowanie animacji pantografów
                       ...
                       pierwszego tłoka
                             pant2x=Parser->GetNextSymbol().ToDouble(); //kąt przesunięcia dla
                       drugiego tłoka
