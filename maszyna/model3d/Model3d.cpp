...
//int TSubModel::SeekFaceNormal(DWORD *Masks, int f,DWORD dwMask,vector3 *pt,GLVERTEX *Vertices)
...
{//Ra: VBO tworzone na poziomie modelu, a nie submodeli
 ...
 if (eType==TP_FREESPOTLIGHT)
 {
  if (!parser.expectToken("nearattenstart:"))
   WriteLogSS("Model light parse failure!", "ERROR");
   ...
   {//Ra: z konwersj¹ na uk³ad scenerii - bêdzie wydajniejsze wyœwietlanie
    ...
    {if (Vertices[i  ].Point==Vertices[i-1].Point ||
     ...
     {//je¿eli punkty siê nak³adaj¹ na siebie
      ...
      WriteLogSS(std::string("Degenerated triangle ignored in: ")+std::string(pName), "ERROR");
     }
     if (i>0) //jeœli pierwszy trójk¹t bêdzie zdegenerowany, to zostanie usuniêty i nie ma co sprawdzaæ
      ...
      {//je¿eli s¹ dalej ni¿ 2km od siebie
       ...
       iNumVerts-=3; //czyli o 3 wierzcho³ki
       i-=3; //wczytanie kolejnego w to miejsce
       WriteLogSS(std::string("Too large triangle ignored in: \"") + std::string(pName), "ERROR");
      }
    }
   }
...
{//dodanie trójk¹tów do submodelu, u¿ywane przy tworzeniu E3D terenu
 ...
 {if (TextureID<=0)
  ...
  //s->asTexture=std::string(TTexturesManager::GetName(tex).c_str());
...
{//zapisanie informacji o submodelu do obiektu pomocniczego
 ...
 {//jeœli ma teksturê niewymienn¹
  ...
  {info->iTexture=++info->iTotalTextures; //przydzielenie numeru tekstury w pliku (od 1)
   std::string t=std::string(pTexture);
   if (t.substr(t.length()-3,4)==".tga")
    t.erase(t.length()-3,4);
   else if (t.substr(t.length()-3,4)==".dds")
    t.erase(t.length()-3,4);
...
void TSubModel::BinInit(TSubModel *s,float4x4 *m,float8 *v,TStringPack *t,TStringPack *n,bool dynamic)
{//ustawienie wskaŸników w submodelu
 ...
 //if (n&&(iName>=0)) asName=std::string(n->String(iName)); else asName="";
 if (n&&(iName>=0))
 {pName=n->String(iName);
  std::string s=std::string(pName);
  if (!s.empty())
  {//jeœli dany submodel jest zgaszonym œwiat³em, to domyœlnie go ukrywamy
   if (s.substr(1,8)=="Light_On") //jeœli jest œwiat³em numerowanym
    iVisible=0; //to domyœlnie wy³¹czyæ, ¿eby siê nie nak³ada³o z obiektem "Light_Off"
   else if (dynamic) //inaczej wy³¹cza³o smugê w latarniach
    if (s.substr(s.length()-2,3)=="_on") //jeœli jest kontrolk¹ w stanie zapalonym
     iVisible=0; //to domyœlnie wy³¹czyæ, ¿eby siê nie nak³ada³o z obiektem "_off"
  }
 }
 else
 ...
 {//obs³uga sta³ej tekstury
  ...
  //asTexture=std::string(t->String(iTexture));
...
{
    std::string tmp=std::string(sName).Trim();
...
{//wczytanie modelu z pliku binarnego
	/*
 WriteLog("Loading - binary model: "+std::string(FileName));
 int i=0,j,k,ch,size;
 TFileStream *fs=new TFileStream(std::string(FileName),fmOpenRead);
 size=fs->Size>>2;
...
{//obrócenie pocz¹tkowe uk³adu wspó³rzêdnych, dla pojazdów wykonywane po analizie animacji
 ...
 {if (iFlags&0x0200) //jeœli wczytano z pliku tekstowego
  ...
  if (!asBinary.empty()) //jeœli jest podana nazwa
  {if (Global::iConvertModels) //i w³¹czony zapis
    SaveToBinFile(stdstrtochar(asBinary)); //utworzy tablicê (m_pVNT)
...
{//zapis modelu binarnego
 ...
 //std::string *asN=&roboczy->asName,*asT=&roboczy->asTexture;
 //roboczy->FirstInit(); //¿eby delete nie usuwa³o czego nie powinno
 TFileStream *fs=new TFileStream(std::string(FileName),fmCreate);
...
/*
void TModel3d::RaRender(vector3 pPosition,double fAngle,GLuint *ReplacableSkinId,int iAlpha)
{
//    glColor3f(1.0f,1.0f,1.0f);
//    glColor3f(0.0f,0.0f,0.0f);
...
/*
void TModel3d::RaRenderAlpha(vector3 pPosition,double fAngle,GLuint *ReplacableSkinId,int iAlpha)
{
 glPushMatrix();
