...
TSwitchExtension::TSwitchExtension(TTrack *owner)
...
TIsolated::TIsolated(const std::string &n,TIsolated *i)
...
TIsolated* TIsolated::Find(const std::string &n)
 ...
 *parser >> token;
 ...
 else
 {
  ...
  WriteLogSS("Unknown track environment: "+str+" ", "ERROR");
 }
 parser->getTokens();
 *parser >> token;
 ...
 *parser >> token;
 ...
 while (str!="endtrack")
 {
  ...
  {//obwód izolowany, do którego tor nale¿y
   parser->getTokens();
   *parser >> token;
   pIsolated=TIsolated::Find(token.c_str());
  }
  else if (str=="angle1")
  {//k¹t œciêcia od strony 1
   parser->getTokens();
   *parser >> a1;
//-   Segment->AngleSet(0,a1); TODO:
  }
  else if (str=="angle2")
  {//k¹t œciêcia od strony 2
   parser->getTokens();
   *parser >> a2;
//-   Segment->AngleSet(1,a2); TODO:
  }
   ...
   *parser >> token;
   ...
   WriteLogSS("Unknown property: " + str +" in track "+ name + "", "ERROR");
...
bool TTrack::AssignEvents(TEvent *NewEvent0,TEvent *NewEvent1,TEvent *NewEvent2)
{
 ...
 if (!Event0)
 {
  ...
  else
  {
   ...
   {
    WriteLogSS("Event0 " + asEvent0Name + " does not exist", "ERROR");
    bError=true;
   }
  }
 }
 ...
 {
  WriteLogSS("Event0 cannot be assigned to track, track already has one", "ERROR");
  bError=true;
 }
 if (!Event1)
 {
  ...
  else
  {
   ...
   {//Ra: tylko w logu informacja
    WriteLogSS("Event1 " +asEvent1Name + " does not exist", "WARNING");
    bError=true;
   }
  }
 }
 ...
 {
  WriteLogSS("Event1 cannot be assigned to track, track already has one", "WARNING");
  bError=true;
 }
 if (!Event2)
 {
  ...
  else
  {
   if (!asEvent0Name.empty())
   {//Ra: tylko w logu informacja
    WriteLogSS("Event2 " + asEvent2Name + " does not exist", "ERROR");
    bError=true;
   }
  }
 }
 ...
 {
	 WriteLogSS("Event2 cannot be assigned to track, track already has one", "ERROR");
  bError=true;
 }
 return !bError;
}
...
bool TTrack::AssignallEvents(TEvent *NewEvent0,TEvent *NewEvent1,TEvent *NewEvent2)
{
 ...
 if (!Eventall0)
 {
  ...
  else
  {
   if (!asEvent0Name.empty())
   {
	 WriteLogSS("Eventall0 " + asEventall0Name + " does not exist", "ERROR");
    bError=true;
   }
  }
 }
 ...
 {
  WriteLogSS("Eventall0 cannot be assigned to track, track already has one", "ERROR");
  bError=true;
 }
 if (!Eventall1)
 {
  ...
  else
  {
   if (!asEvent0Name.empty())
   {//Ra: tylko w logu informacja
    WriteLogSS("Eventall1 " + asEventall1Name + " does not exist", "ERROR");
    bError=true;
   }
  }
 }
 ...
 {
  WriteLogSS("Eventall1 cannot be assigned to track, track already has one", "ERROR");
  bError=true;
 }
 if (!Eventall2)
 {
  ...
  else
  {
   if (!asEvent0Name.empty())
   {//Ra: tylko w logu informacja
    WriteLogSS("Eventall2 " + asEventall2Name + " does not exist", "ERROR");
    bError=true;
   }
  }
 }
 ...
 {
  WriteLogSS("Eventall2 cannot be assigned to track, track already has one", "ERROR");
  bError=true;
 }
 return !bError;
}
...
{//dodanie pojazdu do trajektorii
 ...
 {
  WriteLogSS("Too many dynamics on track "+ pMyNode->asName, "ERROR");
  return false;
 }
};
...
{//usuniêcie pojazdu z listy przypisanych do toru
 ...
 WriteLogSS("Cannot remove dynamic from track", "ERROR");
 return false;
}
...
void TTrack::RaAssign(TGroundNode *gn,TAnimModel *am)
{//Ra: wi¹zanie toru z modelem obrotnicy
 if (eType==tt_Turn)
...
//- for (int i=0;i<iNumDynamics;i++)
//-  Dynamics[i]->Render(); //sam sprawdza, czy VBO; zmienia kontekst VBO!  //TODO:
 //EnvironmentReset();
};
...
{//renderowanie przezroczystych fragmentów pojazdów
 ...
 //EnvironmentSet(); //Ra: pojazdy sobie same teraz licz¹ cienie
//- for (int i=0;i<iNumDynamics;i++)
//-  Dynamics[i]->RenderAlpha(); //sam sprawdza, czy VBO; zmienia kontekst VBO! //TODO:
 //EnvironmentReset();
};
...
{//zapamiêtanie po³¹czen w segmencie
 ...
 WriteLogSS("Cannot set connections", "ERROR");
 return false;
}
...
{//prze³¹czenie torów z uruchomieniem animacji
 ...
 WriteLogSS("Cannot switch normal track", "ERROR");
 return false;
};
