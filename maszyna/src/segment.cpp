...
{//zamiana wspó³rzêdnych na tekst, u¿ywana w b³êdach
 return ftoss(p.x)+" "+ ftoss(p.y)+" "+ ftoss(p.z);
};
...
//--  ErrorLog("Bad geometry: Length <= 0 in TSegment::Init at "+Where(Point1));
 ...
 {//tor ma pojemnoœæ 40 pojazdów, wiêc nie mo¿e byæ za d³ugi
  WriteLogSS("Bad geometry: Length > 500m at "+Where(Point1), "ERROR");
...
//    for (int i = 0; i < iIterations; i++)
    while (true)
    {
        ...
        if (it>10)
        {
         WriteLogSS("Bad geometry: Too many iterations at "+Where(Point1), "ERROR");
