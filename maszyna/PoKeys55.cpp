...
TPoKeys55::TPoKeys55() {
  ...
  iFaza = 0;
  iLastCommand = 0;
  fAnalog[0] = fAnalog[1] = fAnalog[2] = fAnalog[3] = fAnalog[4] = fAnalog[5] =
      fAnalog[6] = -1.0;
...
//---------------------------------------------------------------------------
bool TPoKeys55::Close() { // rozłączenie komunikacji
...
//---------------------------------------------------------------------------
bool TPoKeys55::Connect() { // Ra: to jest do wyczyszcznia z niepotrzebnych
  ...
  // "DIGCF_PRESENT"), which are of the specified class GUID.
  ...
  // of them match our device.
  while (true) {
    InterfaceDataStructure->cbSize = sizeof(SP_DEVICE_INTERFACE_DATA);
    if (SetupDiEnumDeviceInterfaces(DeviceInfoTable, NULL, &InterfaceClassGuid,
                                    InterfaceIndex, InterfaceDataStructure)) {
      ErrorStatus = GetLastError();
      if (ERROR_NO_MORE_ITEMS == ErrorStatus) // Did we reach the end of the
                                              // list of matching devices in the
                                              // DeviceInfoTable?
    ...
    // the VID and PID, which we will then
    ...
    SetupDiGetDeviceRegistryProperty(DeviceInfoTable, &DevInfoData,
                                     SPDRP_HARDWAREID, &dwRegType, NULL, 0,
                                     &dwRegSize);
    ...
    // care about the very first string in the
    // buffer,which will be the "device ID". The device ID is a string which
    // contains the VID and PID,in the example
    ...
    // the memory to prevent potential memory leaks
    ...
    if (MatchFound == true) {
      ...
      // do this,we will need the actual device path first.
      // We can get the path by calling
      // SetupDiGetDeviceInterfaceDetail(),however,we have to call this function
      // twice:  The first
      // time to get the size of the required structure/buffer to hold the
      // detailed interface data,then a second time to actually
      ...
      SetupDiGetDeviceInterfaceDetail(DeviceInfoTable, InterfaceDataStructure,
                                      NULL, NULL, &StructureSize, NULL);
      DetailedInterfaceDataStructure = (PSP_DEVICE_INTERFACE_DETAIL_DATA)(
      ...
      WriteHandle = CreateFile(
          (DetailedInterfaceDataStructure->DevicePath), GENERIC_WRITE,
...
bool TPoKeys55::Write(unsigned char c, unsigned char b3, unsigned char b4,
  ...
  OutputBuffer[0] = 0; // The first byte is the "Report ID" and does not get
                       // transmitted over the USB bus. Always set=0.
  ...
  // read and write to HID class USB devices
  ...
  // Read(); //odczyt trzeba zrobić inaczej - w tym miejscu będzie za szybko i
  // nic się nie odczyta
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
bool TPoKeys55::ReadLoop(int i) { // próbuje odczytać (i) razy
  do {
    if (Read())
      return true;
...
//---------------------------------------------------------------------------
AnsiString TPoKeys55::Version() { // zwraca numer wersji, funkcja nieoptymalna
                                  // czasowo (czeka na odpowiedź)
  if (!WriteHandle)
    return "";
  Write(0x00, 0); // 0x00 - Read serial number, version
  if (ReadLoop(10)) { // 3: serial MSB; 4: serial LSB; 5: software version
                      // (v(1+[4-7]).([0-3])); 6: revision number
    AnsiString s =
        "PoKeys55 #" + AnsiString((InputBuffer[3] << 8) + InputBuffer[4]);
    s += " v" + AnsiString(1 + (InputBuffer[5] >> 4)) + "." +
         AnsiString(InputBuffer[5] & 15) + "." + AnsiString(InputBuffer[6]);
    /* //Ra: pozyskiwanie daty można sobie darować, jest poniekąd bez sensu
      Write(0x04,0); //0x04 - Read build date: drugi argument zmieniać od 0 do
      2, uzyskując kolejno po 4 znaki
      if (ReadLoop(5))
      {//2: 0x04; 3-6: char 1-4, 5-8, 9-11;
      (83-65-112-32-32-49-32-50-48-49-49-0=="Sep  1 2011")
       s+=" ("+AnsiString((char*)InputBuffer+3,4);
       Write(0x04,1); //0x04 - Read build date: drugi argument zmieniać od 0 do
      2, uzyskując kolejno po 4 znaki
       if (ReadLoop(5))
       {s+=AnsiString((char*)InputBuffer+3,4);
        Write(0x04,2); //0x04 - Read build date: drugi argument zmieniać od 0 do
      2, uzyskując kolejno po 4 znaki
        if (ReadLoop(5))
...
bool TPoKeys55::PWM(
    int x, float y) { // ustawienie wskazanego PWM (@12Mhz: 12000=1ms=1000Hz)
  // iPWM[7]=1024; //1024==85333.3333333333ns=11718.75Hz
  iPWM[x] = int(0.5f + 0x0FFF * y) & 0x0FFF; // 0x0FFF=4095
  return true;
}

bool TPoKeys55::Update(bool pause) { // funkcja powinna być wywoływana
                                     // regularnie, np. raz w każdej ramce
                                     // ekranowej
  if (pause) { // specjalna procedura, jeśli utracone połączenie spowodowało
               // pauzę
  ...
  case 1: // odczyt wejść analogowych - komenda i przetwarzanie
    if (iLastCommand !=
        0x3A) // asynchroniczne ustawienie kontrolki może namieszać
      Write(
          0x3A,
          0); // 0x3A - Analog inputs reading – all analog inputs in one command
    else if (Read()) { // jest odebrana ramka i zgodność numeru żądania
      fAnalog[0] =
          ((InputBuffer[21] << 8) + InputBuffer[22]) / 4095.0f; // pin 47
      fAnalog[1] =
          ((InputBuffer[19] << 8) + InputBuffer[20]) / 4095.0f; // pin 46
      fAnalog[2] =
          ((InputBuffer[17] << 8) + InputBuffer[18]) / 4095.0f; // pin 45
      fAnalog[3] =
          ((InputBuffer[15] << 8) + InputBuffer[16]) / 4095.0f; // pin 44
      fAnalog[4] =
          ((InputBuffer[13] << 8) + InputBuffer[14]) / 4095.0f; // pin 43
      fAnalog[5] =
          ((InputBuffer[11] << 8) + InputBuffer[12]) / 4095.0f; // pin 42
      fAnalog[6] = ((InputBuffer[9] << 8) + InputBuffer[10]) / 4095.0f; // pin
                                                                        // 41
  ...
  case 2: // odczyt wejść cyfrowych - komenda i przetwarzanie
    if (iLastCommand !=
        0x31) // asynchroniczne ustawienie kontrolki może namieszać
  ...
  case 3: // ustawienie wyjść analogowych, 0..4095 mapować na 0..65520 (<<4)
    if (Write(0x41, 43 - 1, (iPWM[6] >> 4), (iPWM[6] << 4))) // wysłanie
                                                             // ustawień
      iRepeats = 0; // informacja, że poszło dobrze
  ...
  else if (iRepeats >= 10) // youBy 2014-07: przy 5 powtórzeniach sieje mi pauzą
                           // po 2 razy na sekundę, a przy 10 jest ok
