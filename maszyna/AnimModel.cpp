...
bool TAnimModel::Init(std::string asName, std::string asReplacableTexture)
{
 ...
 return (Init(TModelsManager::GetModel(stdstrtochar(asName))));
}
 ...
 {if (str!="notload")
  {//gdy brak modelu
   ...
   {if (str.substr(str.length()-3,4)==".t3d")
    ...
    WriteLogSS("Terrain model "+str+" will be created.", "INFO");
   }
   else
    WriteLogSS("Missed file: "+str, "ERROR");
  }
 }
 else
 ...
 *parser >> token;
  ...
  *parser >> token;
  ...
  do
  {
   ti = atof(str.c_str()); //stan œwiat³a jest liczb¹ z u³amkiem //TODO: CZY DOBRZE?
   LightSet(i,ti);
   i++;
   parser->getTokens();
   *parser >> token;
...
/*
 ...
 if ((char*)pData=="Vocaloid Motion Data 0002")
 {
  ...
  {if (strcmp(pAdvanced->pMovementData[i].cBone,name.c_str()))
   {//jeœli pozycja w tabelce nie by³a wyszukiwana w submodelach
    ...
    name=pAdvanced->pMovementData[i].cBone; //nowa nazwa do pomijania
   }
