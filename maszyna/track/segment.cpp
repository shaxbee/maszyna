...
bool  TSegment::Init(vector3 NewPoint1, vector3 NewCPointOut, vector3 NewCPointIn, vector3 NewPoint2, double fNewStep,
...
{//wersja dla krzywej
    ...
    {//Ra: prosty dostanie podzia³, jak ma wpisane kontrolne :(
        double s=0;
        int i=0;
        fTsBuffer= new double[std::ceil(fLength/fStep)+1];
...
float  TSegment::RombergIntegral(double fA, double fB)
{
    double fH = fB - fA;
...
float  TSegment::GetTFromS(double s)
{
...
double  TSegment::ComputeLength(vector3 p1, vector3 cp1, vector3 cp2, vector3 p2)  //McZapkie-150503: dlugosc miedzy punktami krzywej
{
    double t,l=0;
    vector3 tmp,last= p1;
...
void  emissionoff()
{
 float emmm2[]= { 0,0,0,0 };
        ...
        glMaterialfv(GL_FRONT_AND_BACK,GL_EMISSION, emmm2);
...
void  TSegment::RenderLoft(const vector3 *ShapePoints, int iNumShapePoints,
...
//           step= SquareMagnitude(Global::GetCameraPosition()+pos);
           ...
           {//gdy przekroczyliœmy koniec - st¹d dziury w torach...
               ...
               s=fLength;
               i=std::ceil(fLength/fStep); //20/5 ma dawaæ 4
               m2=1.0; jmm2=0.0;
           }
...
// LUKI I FLEXT LUKOWE
       glBegin(GL_TRIANGLE_STRIP);
         if (trapez)
          ...
          {
		   if ((j >4) && (j < 7)) glColor3f(1.0f,0.0f,0.0f);
    ...
    {//gdy prosty
            ...
            if (trapez) 
                for (j=0; j<iNumShapePoints; j++)
                {
				// PUNKTY 5 I 6 TO POWIERZCHNIA SZYNY	
				//if ((j >4) && (j < 7)) glColor3f(0.0f,0.0f,1.0f);
				//else glColor3f(1.0f,1.0f,1.0f);
...
// TYLKO POWIERZCHNIA SZNY
			...
			if (!trapez) 
			{
				...
				//if ((j >4) && (j < 7)) glColor3f(1.1f,1.0f,0.0f);
				//else glColor3f(1.0f,1.0f,1.0f);
...
void  TSegment::RenderSwitchRail(const vector3 *ShapePoints1, const vector3 *ShapePoints2,
        ...
        int i;
            if (bCurve)
            {
                ...
                glBegin(GL_LINE_STRIP);
                    for (int i=0; i<=16; i++)
                    {
                        pt= FastGetPoint(double(i)/16.0f);
                        glVertex3f(pt.x,pt.y,pt.z);
                    }
                 glEnd();

				 
				drawSphere3(Point2.x, Point2.y, Point2.z-0.1, 0.06, 9, 3); //red
				drawSphere3(Point1.x, Point1.y, Point1.z+0.1, 0.06, 9, 2); //green
            ...
            else // JEZELI PROSTY ...
            {
				...
				//drawSphere3(Point2.x, Point2.y, Point2.z, 0.1, 6, 1);
                ...
                glBegin(GL_LINE_STRIP);
                    ...
                    glVertex3f(Point2.x+CPointIn.x, Point2.y+CPointIn.y, Point2.z+CPointIn.z);
                glEnd();
//-drawSphere3(Point1.x+CPointOut.x, Point1.y+CPointOut.y, Point1.z+CPointOut.z, 0.1, 9, 4); //green
